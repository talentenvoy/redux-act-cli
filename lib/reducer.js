...
const initialState = {
  ...
  errorFetching${flc(model)}${plural ? 's' : ''}: false,
  totalCount: 0,
...
const reducer = {
  [cancelOperation]: state => spreadState(state, {
    creating${flc(model)}: false,
    editing${flc(model)}: null,
    deleting${flc(model)}: null,
  }),
  ...
  [fetch${flc(model)}${plural ? 's' : ''}Success]: (state, { data: ${model}${plural ? 's' : ''}, totalCount }) => spreadState(state, {
    ${model}${plural ? 's' : ''},
    totalCount,
    isFetching${flc(model)}${plural ? 's' : ''}: false,
  }),
  [fetch${flc(model)}${plural ? 's' : ''}Error]: state => spreadState(state, {
    errorFetching${flc(model)}${plural ? 's' : ''}: true,
    isFetching${flc(model)}${plural ? 's' : ''}: false,
  }),
  ...
  [edit${flc(model)}Success]: (state, edited${flc(model)}) => spreadState(state, () => {
    ...
    const index = ${model}${plural ? 's' : ''}
      .findIndex((${model}) => ${model}.id === edited${flc(model)}.id);
    return {
      ...state,
      ${model}${plural ? 's' : ''}: [
